...
using KrSpoken.WordAndSentences;

namespace KrSpoken
{
    public delegate void FromThreadRequestInvokeDelegate();
    public partial class FrmPractice : Form
    {
       ...
       public FrmMain frmMain;
        ...
        private WordCaptureRuntime WcRuntime;//划词搜索模块 
        private bool WordCapture_IsOpen = false;//判断是否已经打开划词搜索窗口
        private bool WordCapture_On = true; //是否启用划词搜索
        ...
        private void FrmPractice_Load(object sender, EventArgs e)
        {
            ...
            {
                PlayerList.Font = new Font("微软雅黑", 9);
                ScriptBox.Font = new Font("微软雅黑", 14);
                TipLab.Font = new Font("微软雅黑", 9);
                PlayStateLabel.Font=new Font("微软雅黑", 10);
                PlayTimeLabel .Font=new Font("微软雅黑", 10);
            }
            try
            {
                WcRuntime = new WordCaptureRuntime(AppProfile.WordCaptureDatabasePath);
                WcRuntime.FrmClose += new OnFrmWordClosingDelegate(WcRuntime_FrmClose);
            }
            catch(Exception ex)
            {
                MessageBox.Show(ex.Message);
                WordCapture_On = false;
            }
            this.Text=pTitle + "  口语练习模式(试题包类型:"+pType+")";
            PlayerList.Nodes[0].Text = pTitle;
            PlayerList.Nodes[1].Expand();
            PlayerList.Nodes[2].Expand();
            PlayerList.Nodes[3].Expand();
            StartLoad();
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (PlayData.SubItemID == 0)
                    {
                        ...
                        PlayerList.SelectedNode = PlayerList.Nodes[NowItem];
                    }
                    else
                    {
                        NowItem = PlayData.ItemID;
                        NowSubItem = PlayData.SubItemID;
                        PlayerList.SelectedNode = PlayerList.Nodes[NowItem].Nodes[NowSubItem - 1];
                    }
                    string file = "";
        ...
        //树列表项目点击事件
        private void PlayerList_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            if (IsRecoding)
            {
                MessageBox.Show("录音模式不允许切换操作，请等待录音结束", "警告", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                //节点选择
                if (PlayData.SubItemID == 0)
                {
                    NowItem = PlayData.ItemID;
            ...
            {
                TreeListClickItemID = FindIdFromItemID(e.Node.Index);
                StopPlayAny();
                ThisItemPlayEnd = 6;
            }
            else
            {
                TreeListClickItemID = FindIdFromItemID(e.Node.Parent.Index,e.Node.Index+1);
                StopPlayAny();
                ThisItemPlayEnd = 6;
            }
        ...
        private void RestartBut_Click(object sender, EventArgs e)
        {
            if (IsRecoding)
            {
                MessageBox.Show("录音模式不允许重复进行，请等待录音结束","警告",MessageBoxButtons.OK,MessageBoxIcon.Warning);
                return;
            }
            StopPlayAny();
            ThisItemPlayEnd = 5;
        ...
        //重置按钮悬停效果
        private void RestartBut_MouseHover(object sender, EventArgs e)
        {
            RestartBut.Image = PlayerButton.But_RePlay_2;
        }
        //重置按钮悬停效果
        private void RestartBut_MouseLeave(object sender, EventArgs e)
        {
            RestartBut.Image = PlayerButton.But_RePlay_1;
        }
        /*列表按钮悬停效果
        private void ListBut_MouseHover(object sender, EventArgs e)
        {
            ListBut.Image = PlayerButton.But_List_2;
        }
        //列表按钮悬停效果
        private void ListBut_MouseLeave(object sender, EventArgs e)
        {
            ListBut.Image = PlayerButton.But_List_1;
        }
        //列表按钮按下效果
        private void ListBut_Click(object sender, EventArgs e)
        {
            ListBut.Image = PlayerButton.But_List_3;
            MyMenu.Show(ListBut, new Point(0, 25));
        }
         */
        //停止当前播放 
        ...
        #region  划词搜索部分
        //划词搜索窗口关闭时的事件
        private void WcRuntime_FrmClose()
        {
            WordCapture_IsOpen = false;
        }
        //划词搜索实现模块
        private void ScriptBox_MouseUp(object sender, MouseEventArgs e)
        {

            if (WordCapture_IsOpen == true)
            {
                WcRuntime.CloseFrm();
                WordCapture_IsOpen = false;
            }
            if (!WordCapture_On) { return; }
            if (ScriptBox.SelectedText != "")
            {
                if (ScriptBox.SelectedText.Replace("\r", "").Replace("\n", "").Trim().IndexOf(" ") < 0) //判断为单词
                {
                    WcRuntime.OpenWordCapture(ScriptBox.SelectedText.Replace("\r", "").Replace("\n", "").Trim());
                    WordCapture_IsOpen = true;
                }
            }
        }
        #endregion
